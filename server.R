...
server <- function(input, output, session) {
  ...
  output$timeline2 <- renderDygraph({
    ...
    isolate({
      ...
      temp_db_draw$TtM <- as.numeric(difftime(as.Date(input$ti_Expiration_Date2), as.Date(temp_db_draw$Pricing_Date), unit="weeks"))/52.25
      temp_db_draw$TtM <- round(temp_db_draw$TtM, digits = 2)
      ...
      temp_db_draw$Nd1 <- pnorm(temp_db_draw$d1,lower.tail = TRUE)
      ...
      temp_db_draw$Asset <- temp_db_draw$Stock_Price * temp_db_draw$Nd1
      temp_db_draw$'Forward Value' <-
        round(temp_db_draw$Liability + temp_db_draw$Asset, 1)
